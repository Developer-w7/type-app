...
export default function AddNewJob() {
  ...
  const onSubmit: SubmitHandler<Inputs> = (data) => {
    ...
    const submitData = async () => {
        try {
            const response = await axiosPrivate.post('http://127.0.0.1:5000/add_job',
...
useEffect(() => {
    const fetchData = async () => {
        try {
            const response = await axiosPrivate.get('http://127.0.0.1:5000/users?page=1&limit=10',
                {   
                    headers: { 'Content-Type': 'application/json' },
                    withCredentials: true
                }
...
<div>
<span> <h4>Add Service Page</h4></span>
/* "handleSubmit" will validate your inputs before invoking "onSubmit" */
    ...
    >
      ...
      <Controller
        ...
        render={({ field }) => <TextField id="outlined-basic" fullWidth label="Title" variant="outlined" {...field}/>}
      />
      ...
      <Controller
        ...
        control={control}
        rules={{ required: true ,maxLength: 20}}
        render={({ field }) => <TextField id="outlined-basic" fullWidth label="Desc" variant="outlined" {...field}/>}
      />
